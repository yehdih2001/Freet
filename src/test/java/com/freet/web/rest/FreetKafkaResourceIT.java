...
class FreetKafkaResourceIT {
    ...
    void setup() {
        ...
        FreetKafkaResource kafkaResource = new FreetKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/freet-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/freet-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
